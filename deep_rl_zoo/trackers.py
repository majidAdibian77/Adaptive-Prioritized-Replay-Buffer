...
class TensorboardEvalEpisodeTracker(EpisodeTracker):
    """Extend EpisodeTracker to write to tensorboard"""

    def __init__(self, writer: SummaryWriter):
        super().__init__()
        self._eval_steps = 0  # keep track total number of steps in eval, does not reset
        self._train_step = 0  # keep train stap that evaluation is run in that stap
        self._eval_episodes = 0  # keep track total number of episodes in eval, does not reset
        self._writer = writer
    ...
    def step(self, env, timestep_t, agent, a_t) -> None:
        ...
        if timestep_t.done:
            ...
            # tracker per step
            self._writer.add_scalar('evaluation/num_episodes', self._eval_episodes, self._eval_steps)
            self._writer.add_scalar('evaluation/episode_return', episode_return, self._eval_steps)
            self._writer.add_scalar('evaluation/episode_steps', episode_step, self._eval_steps)
            # save train step that this eavluations is run in that step
            self._writer.add_scalar('evaluation/train_stap', self._train_step, self._eval_steps)

            # For Atari games like MontezumaRevenge and Pitfall
            if isinstance(timestep_t.info, dict) and 'episode_visited_rooms' in timestep_t.info:
                episode_visited_rooms = self._episode_visited_rooms[-1]
                self._writer.add_scalar('evaluation/episode_visited_rooms', episode_visited_rooms, self._eval_steps)
...
def make_default_trackers(log_dir=None, debug_screenshots_interval=0, eval=False):
    """
    ...
    if log_dir:
        ...
        trackers = [
            TensorboardEvalEpisodeTracker(writer) if eval else TensorboardEpisodeTracker(writer)
