...
class PrioritizedReplay:
    ...
    def __init__(
        ...
        ## Contribution 1: Use rewards of samples in priortize replay buffer
        use_reward_in_priortized_replay: bool = False,
        ## Contibution 2: Use counter in priortize replay buffer
        use_counter: bool = False,
        ## Contribution 3: Use Pi(a|s) of samples in priortize replay buffer
        use_Pi_in_priortize_replay: bool = False,
    ):
        if capacity <= 0:
            raise ValueError(f'Expect capacity to be a positive integer, got {capacity}')
        self.structure = structure
        ...
        ## Contribution 1: Use rewards of samples in priortize replay buffer
        self.use_reward_in_priortized_replay = use_reward_in_priortized_replay
        self.reward_range = [0,1] ## mean and std of rewards
        ## Contibution 2: Use counter in priortize replay buffer
        self.use_counter = use_counter
        self._transition_counter = np.ones((capacity,))
        ## Contribution 3: Use Pi(a|s) of samples in priortize replay buffer
        self.use_Pi_in_priortize_replay = use_Pi_in_priortize_replay
        
        
    def add(self, item: ReplayStructure) -> None:
        """Adds a single item with a given priority to the replay buffer."""
        index = self._num_added % self._capacity
        self._priorities[index] = self._max_seen_priority
        self._storage[index] = self._encoder(item)
        self._num_added += 1

        ## Contibution 2: Use counter in priortize replay buffer
        if self.use_counter:
            self._transition_counter[index] = 1
    ...
    def sample(self, size: int) -> Tuple[ReplayStructure, np.ndarray, np.ndarray]:
        ...
        if self.priority_exponent == 0:
            ...
            priorities = self._priorities[: self.size] ** self.priority_exponent
    ...
    def project_to_new_band(self, prob, lower_band=0.1, uper_band=0.9):
        """ 
        Project a numper in (0,1) to a number in (lower_band, uper_band) by shiftting to 0 and squeeze it and then shift back. 
        So if "b" be squeeze coeficient and a be shift number:
        (0,1) -> (-a,1-a) -> (-ab,b-ab) -> (-ab+a,b-ab+a)=(lower_band, upper_band) -> "b = uper_band-lower_ban" and "a = lower_band/(1-b)"
        """
        b = uper_band-lower_band ## b: squeeze coeficient
        a = lower_band/(1-b) ## a: shift number
        new_prob = (prob-a)*b + a
        return new_prob
    
    def update_priorities(self, indices: Sequence[int], td_error: Sequence[float], transitions: Transition, Q_s1=None) -> None:
        """Updates indices with given priorities."""
        exponent = []

        ## Contribution 1: Use rewards of samples in priortize replay buffer
        if self.use_reward_in_priortized_replay:
            new_prob = 0.999**self._num_added
            old_prob = 1-new_prob
        ...
        ## Contribution 3: Use Pi(a|s) of samples in priortize replay buffer
        if self.use_Pi_in_priortize_replay:
            a_tm1 = torch.from_numpy(transitions.a_tm1).to(device=Q_s1.device, dtype=torch.int64)  # [batch_size]
            actions_probs = torch.nn.functional.softmax(Q_s1)
            # actions_probs = torch.clip(Q_s1, 0) / torch.clip(Q_s1, 0.001).sum(dim=1).reshape((-1,1))
            Pi_s1_a = actions_probs[torch.arange(actions_probs.shape[0]), a_tm1] ## π(a|s1)
            Pi_s1_a = torch.detach(Pi_s1_a).cpu().numpy()
            # Pi_s1_a = torch.detach(Pi_s1_a).cpu().numpy() + 0.1
            # Pi_s1_a = Pi_s1_a ^ (1 - 0.9999^self._step_t)
            # Pi_s1_a = self.project_to_new_band(Pi_s1_a, 0.1, 0.9)
            exponent.append(Pi_s1_a)
        ...
        if len(exponent)>0:
            ...
            priorities = priorities ** exponent

        self._max_seen_priority = max([self._max_seen_priority, priorities.max()])
    ...
    def priority_exponent(self):
        """Importance sampling exponent at current step."""
        if type(self._priority_exponent) == float:
            return self._priority_exponent
        else:
            return self._priority_exponent(self._num_added)
