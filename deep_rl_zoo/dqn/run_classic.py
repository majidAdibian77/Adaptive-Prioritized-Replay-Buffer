...
from deep_rl_zoo.schedule import ExponentialSchedule
...
"""
python3 -m deep_rl_zoo.dqn.run_classic --run_name "original" --environment_name=LunarLander-v2 --replay_capacity 100000 \
                ...
                --num_eval_episods 10  --max_eval_episod_steps 10000 --save_ckpt_iter 20
"""
...
def main(argv):
    ...
    train_agent = agent.Dqn(
        ...
        agent_name = f"DQN-{FLAGS.run_name}-{FLAGS.environment_name}",
    )
    ...
    eval_agent = greedy_actors.EpsilonGreedyActor(
        ...
        random_state=random_state,
        device=runtime_device,
        name=f'DQN-{FLAGS.run_name}-{FLAGS.environment_name}-greedy-eval',
    ...
    main_loop.run_single_thread_training_iterations(
        ...
        csv_file=os.path.join(FLAGS.results_csv_path, f"dqn_{FLAGS.run_name}-{FLAGS.environment_name}_results.csv"),
    )
