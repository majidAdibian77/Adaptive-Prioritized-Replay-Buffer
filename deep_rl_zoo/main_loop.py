...
def run_eval_env_loop(
    agent: types_lib.Agent, env: gym.Env, num_episods=math.inf, max_episod_steps=math.inf
) -> Iterable[Tuple[gym.Env, types_lib.TimeStep, types_lib.Agent, types_lib.Action]]:

    if not isinstance(agent, types_lib.Agent):
        raise RuntimeError('Expect agent to be an instance of types_lib.Agent.')

    for _ in range(num_episods):  # For each episode.
        agent.reset()
        # Think of reset as a special 'action' the agent takes, thus given us a reward 'zero', and a new state 's_t'.
        observation = env.reset()
        reward = 0.0
        ...
        while True:  # For each step in the current episode.
            ...
            observation, reward, done, info = env.step(a_tm1)
            first_step = False
            if done or episod_steps>max_episod_steps:  # Actual end of an episode
                # This final agent.step() will ensure the done state and final reward will be seen by the agent and the trackers
                timestep_t = types_lib.TimeStep(
                    observation=observation,
                    reward=reward,
...
def run_eval_env_steps(num_episods: int, agent: types_lib.Agent, env: gym.Env, trackers: Iterable[Any], max_episod_steps=math.inf) -> Mapping[Text, float]:
    seq = run_eval_env_loop(agent, env, num_episods, max_episod_steps)
...
def run_single_thread_training_iterations(
    ...
    max_eval_episod_steps: int = math.inf,
    save_ckpt_iter: int = 1
) -> None:
    ...
    eval_trackers = None
    if num_eval_episods > 0 and eval_agent is not None and eval_env is not None:
        should_run_evaluator = True
        eval_tb_log_prefix = (
            get_tb_log_prefix(eval_env.spec.id, eval_agent.agent_name, tag, 'eval') if use_tensorboard else None
        )
        eval_trackers = trackers_lib.make_default_trackers(eval_tb_log_prefix, debug_screenshots_interval, eval=True)
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        if should_run_evaluator is True:
            ...
            for eval_tracker in eval_trackers:
                eval_tracker._train_step = iteration * num_train_steps

            # Run some evaluation steps.
            eval_stats = run_eval_env_steps(num_eval_episods, eval_agent, eval_env, eval_trackers, max_eval_episod_steps)

            # Logging evaluation statistics.
            eval_output = [
                ('eval_episods', iteration * num_eval_episods, '%5d'),
